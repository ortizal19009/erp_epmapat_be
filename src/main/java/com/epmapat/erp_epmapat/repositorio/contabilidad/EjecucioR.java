...
public interface EjecucioR extends JpaRepository<Ejecucio, Long> {
   ...
   // Contar los compromisos de una partixcerti (para no eliminar partixcerti)
   short countByIdparxcer(Long idparxcer);

   // Contar las Partidas de una ejecución para disabled eliminar partida
   ...
   // Suma los prmisos de un idtrami (para actualizar tramipresu.totprmiso)
   @Query("SELECT COALESCE(SUM(e.prmiso), 0) FROM Ejecucio e WHERE e.idtrami = :idtrami")
   BigDecimal sumaPrmisoPorIdtrami(@Param("idtrami") Long idtrami);

   // Suma los prmisos de un idparxcer (para actualizar partixcerti.totprmisos)
   @Query("SELECT COALESCE(SUM(e.prmiso), 0) FROM Ejecucio e WHERE e.idparxcer = :idparxcer")
   BigDecimal sumaPrmisoPorIdparxcer(@Param("idparxcer") Long idparxcer);

   // Suma los prmisos de un intpre (para actualizar presupue.totmisos)
   @Query("SELECT COALESCE(SUM(e.prmiso), 0) FROM Ejecucio e WHERE e.intpre.intpre = :intpre")
   ...
   // Ultima Fecha
   @Query("SELECT e.fecha_eje FROM Ejecucio e ORDER BY e.fecha_eje DESC")
   List<LocalDate> findLastFechaEjeRaw();

   default LocalDate findLastFechaEje() {
      List<LocalDate> fechas = findLastFechaEjeRaw();
      return fechas.isEmpty() ? null : fechas.get(0);
   }

   // Suma los devengados de un compromiso
   @Query("SELECT COALESCE(SUM(e.devengado), 0) FROM Ejecucio e WHERE e.idprmiso = :idprmiso")
   BigDecimal sumaDevengadoPorIdprmiso(@Param("idprmiso") Long idprmiso);
