...
public interface EmisionIndividualR extends JpaRepository<EmisionIndividual, Long>{
    ...
    @Query(value = "select r.idrubro_rubros,rs.descripcion,  count(*) as nrofacturas,  sum(r.valorunitario * r.cantidad) as total from emisionindividual ei join lecturas l on ei.idlecturanueva = l.idlectura join rubroxfac r on l.idfactura = r.idfactura_facturas join rubros rs on r.idrubro_rubros = rs.idrubro where ei.idemision = ?1 and not r.idrubro_rubros = 5 group by r.idrubro_rubros, rs.descripcion ", nativeQuery = true)
    public List<Object[]> findLecturasNuevas(Long idemision); 
    /* REPORTE DE LECTURAS ANTERIORES */
    @Query(value = "select r.idrubro_rubros,rs.descripcion,  count(*) as nrofacturas,  sum(r.valorunitario * r.cantidad) as total from emisionindividual ei join lecturas l on ei.idlecturaanterior = l.idlectura join rubroxfac r on l.idfactura = r.idfactura_facturas join rubros rs on r.idrubro_rubros = rs.idrubro where ei.idemision = ?1 and not r.idrubro_rubros = 5 group by r.idrubro_rubros, rs.descripcion ", nativeQuery = true)
