...
public class RetencionProcesamientoSRIService {
   ...
   private static final int MAX_INTENTOS_AUTORIZACION = 3;
   ...
   private AutorizacionResultado enviarFirmarYAutorizar(String xmlPlano) {
         ...
         try {
            ...
            if (response.getStatusCode().value() == 202 || contienePendiente(body)) {
               if (intento < MAX_INTENTOS_AUTORIZACION) {
                  esperarAntesDeReintentar(intento, "La autorizacion aun no esta disponible");
                  continue;
               }
      ...
      return AutorizacionResultado.pendiente(
            "No fue posible obtener el XML autorizado de la retencion",
            0,
            "");
   }
   ...
   private void esperarAntesDeReintentar(int intento, String motivo) {
      long esperaMs = 1000L * intento;
      try {
      ...
      if (StringUtils.hasText(motivo)) {
         System.out.println("[SRI] Reintentando autorizacion de retencion (" + intento + "/3): " + motivo);
      }
   }
   ...
   private static final class AutorizacionResultado {
      ...
      private AutorizacionResultado(boolean pendiente, String xmlAutorizado, String mensaje, int codigoHttp,
            String detalle) {
      ...
      private static AutorizacionResultado autorizado(String xmlAutorizado) {
         return new AutorizacionResultado(false, xmlAutorizado, null, 200, null);
      }

      private static AutorizacionResultado pendiente(String mensaje, int codigoHttp, String detalle) {
         return new AutorizacionResultado(true, null, mensaje, codigoHttp, detalle);
      }
   }
}
